...
init -2 python:
    ...
    class shopper(store.object):
        ...
        def __init__(self, name, budget = 0, trust = 0, pref_hag= 0, max_hag= 0, min_hag= 0, fav_turn= 0, random = 2, items=[], img=""):
            global all_shoppers
            self.name = name
            self.budget = budget
            self.bprice  = brpice
            ...
            slef.min_hag = min_hag
            slef.fav_turn = fav_turn
            self.items = items
...
#CREAR COMPRADORES
init python:
    Girl = shopper("Girl", budget = 3000, pref_hag = 140, max_hag = 180, min_hag 40, fav_turn = 0, items=[apple])
